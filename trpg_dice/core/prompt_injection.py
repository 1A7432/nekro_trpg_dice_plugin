...
async def inject_trpg_system_prompt(_ctx) -> str:
    ...
    prompt_parts = [
        ...
        "• query_knowledge_pool(query, pool_type='keeper') - 关键词搜索模组知识池（AGENT:结果只给AI观察）。支持多关键词空格分隔",
        "• list_module_elements(element_type='scenes') - 列出模组元素清单：scenes/npcs/clues/truths/timeline（AGENT:结果只给AI观察）",
        "• get_module_element_detail(element_type, name) - 获取单个场景/NPC/线索的完整详情（AGENT:结果只给AI观察，解决内容过长截断问题）",
        "• inspect_knowledge_pool(pool_type='keeper') - 直接查看知识池全部原始内容（AGENT:结果只给AI观察，内容过多时优先用 list + get_detail 组合）",
