...
async def inject_trpg_system_prompt(_ctx) -> str:
    ...
    prompt_parts = [
        ...
        "• get_module_summary() - 开局前获取模组全局概要：summary/background/truths/timeline/场景清单/NPC清单/威胁清单（AGENT:结果只给AI观察）",
        ...
        "• inspect_knowledge_pool(pool_type='keeper') - dump 知识池全部原始内容（AGENT:结果只给AI观察，内容过多时优先用 list + get_detail）",
