...
我们实现了6个核心提示词注入方法，每个都针对不同的场景和需求：
...
### 6. **历史记忆** (`session_history_memory`)
- **目的**: 加载上次跑团的战报摘要，实现剧情连续性
- **内容**: 上次跑团的玩家、评分、关键事件、游戏进度
- **触发**: 每次对话时，如果有历史战报则注入
...
💡 *以上是上次跑团的简要回顾，请基于此继续推进剧情*
```

**AI的表现:**
AI会了解上次跑团的关键剧情进展，知道调查员们已经找到了地下遗迹入口，会自然地继续推进剧情，而不是从头开始。同时会根据上次的表现调整难度和氛围。
