...
NekroAgent 的架构中，**命令系统（`/` 指令）和 AI 对话系统是完全隔离的两条通道**。理解这一点对跑团体验至关重要。

#### 为什么会这样？

```
QQ消息 → NekroAgent → detect_command("/ra 侦查") → 命令处理器执行
                              ↓                          ↓
                        不进入 AI 上下文              输出广播到 WebUI/QQ
```

- 命令输出通过 `command_output_broadcaster` 实时推送给前端，**不做持久化**
- AI 的上下文只包含经过 `push_human_message` 的消息
- 因此 **AI 完全看不到玩家输入的命令，也看不到命令的输出结果**

#### 对跑团的影响

| 方式 | AI 是否看到 | 说明 |
|------|------------|------|
| 玩家输入 `/ra 侦查` | ❌ 看不到 | AI 不知道玩家掷了骰，也不知道结果 |
| AI 调用 `skill_check` | ✅ 看得到 | AI 自己调用工具，自然知道结果 |
| 玩家说"我侦查一下" | ✅ 看得到 | 作为自然语言消息进入 AI 上下文 |

#### 推荐的跑团玩法

**方式一：AI 代掷（最流畅）**
- 玩家说"我想侦查一下这个房间"
- AI KP 自动调用 `skill_check` 工具掷骰
- AI 直接根据结果推进剧情
- 适合：追求流畅体验的线上文字跑团

**方式二：玩家掷骰 + 手动告知 AI**
- 玩家输入 `/ra 侦查`（或 `.ra 侦查`，需改命令前缀）
- 看到结果后，复制结果并 `@AI` 或继续对话
- 例：`@AI 我刚掷了侦查，结果是51，普通成功`
- 适合：喜欢亲手掷骰、有仪式感的玩家
...
NekroAgent 默认命令前缀是 `/`，所以 `.ra 侦查` 不会被识别为命令。

如需使用 `.` 前缀（跑团圈习惯）：
- 进入 NekroAgent Web UI → 配置 → OneBot V11 适配器 → 命令前缀
- 将 `/` 改为 `.`
- 重启容器生效

> ⚠️ **注意**：即使改成 `.` 前缀，命令输出仍然不会被 AI 看到，这只是改变了触发命令的符号。
