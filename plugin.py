...
# 导入核心模块
from .trpg_dice.core.dice_engine import DiceParser, DiceRoller, DiceResult, config as dice_config
from .trpg_dice.core.character_manager import CharacterManager, CharacterSheet, CharacterTemplate
from .trpg_dice.core.document_manager import VectorDatabaseManager, DocumentProcessor
from .trpg_dice.core.prompt_injection import register_prompt_injections
...
async def handle_hidden_roll(event: MessageEvent, args: Message = CommandArg()):
    ...
    try:
        ...
        try:
            # 直接通过用户ID发送私聊消息
            await message.send(f"🎲 {result.format_result()}", user_id=event.user_id)
        except Exception:
